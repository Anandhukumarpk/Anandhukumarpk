...
def placeorder(request):
    ...
    if request.method == 'POST':
        ...
        cart_total = request.session.get('cart_total', 0)  # assuming you store the total in the session

        # Create a new PlaceOrder object and save it
        PlaceOrder.objects.create(
            user=request.user,  # assuming there's a user field in the PlaceOrder model
            name=name,
            address=address,
            district=district,
            pincode=pincode,
            country=country,
            contact=contact,
            total_price=cart_total  # assuming the model has a field for total price
        )
